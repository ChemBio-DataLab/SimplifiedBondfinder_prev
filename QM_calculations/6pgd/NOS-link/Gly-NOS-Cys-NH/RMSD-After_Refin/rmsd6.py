...
# Define atom indices for RMSD calculations
backbone_gly_indices = [0, 1, 2, 3]         # Gly backbone
sidechain_gly_indices = []                  # Gly sidechain (empty)
backbone_cys_indices = [6, 7, 8, 9, 12]     # Cys backbone
sidechain_cys_indices = [10, 11]              # Cys sidechain
...
def align_structures(initial_atoms, optimized_atoms):
    """
    Aligns optimized_atoms to initial_atoms using the Kabsch algorithm.
    Returns the aligned optimized_atoms coordinates.
    """
    # Extract coordinates
    P = np.array([atom[1] for atom in initial_atoms])
...
def main():
    ...
    optimized_gjf = '6pgd-NHOS_QM-optimized-NoH.gjf'
    output_gjf = 'aligned_structure.gjf'
    output_xyz = 'aligned_structure.xyz'
    initial_xyz = 'After_Refine-NoH.xyz'

    # Parse the .gjf files
    initial_atoms = parse_gjf(Refined_stru)
    optimized_atoms = parse_gjf(optimized_gjf)

    # Debugging: Print number of atoms parsed
    print(f"Initial structure parsed: {len(initial_atoms)} atoms.")
    print(f"Optimized structure parsed: {len(optimized_atoms)} atoms.")

    # Check if the number of atoms and their symbols match
    if len(initial_atoms) != len(optimized_atoms):
        print("Error: The two structures have different numbers of atoms.")
        return
    for i, (atom1, atom2) in enumerate(zip(initial_atoms, optimized_atoms)):
        if atom1[0] != atom2[0]:
    ...
    try:
        aligned_coords = align_structures(initial_atoms, optimized_atoms)
    ...
    # Extract header and charge/multiplicity from the initial .gjf file
    ...
    # Optionally, write the initial structure to an .xyz file for comparison
    write_xyz(initial_xyz, initial_atoms, comment='Initial Structure')
    print(f"Initial structure has been written to {initial_xyz}")

    # Calculate RMSD for specified atom groups
    # Convert aligned_atoms to a numpy array for easier indexing
    aligned_coords_array = np.array([atom[1] for atom in aligned_atoms])
    initial_coords_array = np.array([atom[1] for atom in initial_atoms])

    rmsd_results = {}

    # Function to handle RMSD calculation safely
    def compute_rmsd(group_name, indices):
        if not indices:
            rmsd_results[group_name] = None
            return
        try:
            coords_initial = initial_coords_array[indices]
            coords_aligned = aligned_coords_array[indices]
            rmsd = calculate_rmsd(coords_initial, coords_aligned)
            rmsd_results[group_name] = rmsd
        except IndexError:
            print(f"Error: One or more indices for {group_name} are out of range.")
            rmsd_results[group_name] = None

    # Define the groups
    groups = {
        'Gly Backbone': backbone_gly_indices,
        'Gly Sidechain': sidechain_gly_indices,
        'Cys Backbone': backbone_cys_indices,
        'Cys Sidechain': sidechain_cys_indices
    }

    # Compute RMSD for each group
    for group_name, indices in groups.items():
        compute_rmsd(group_name, indices)

    # Display RMSD results
    print("\nRMSD Results:")
    for group, rmsd in rmsd_results.items():
        if rmsd is not None:
            print(f"  {group}: {rmsd:.4f} Å")
        else:
            print(f"  {group}: N/A (No atoms in this group)")

    # Optionally, calculate overall RMSD for all atoms
    overall_rmsd = calculate_rmsd(initial_coords_array, aligned_coords_array)
