...
def reorder_atoms(refined_atoms, optimized_atoms):
    """
    Reorders the atoms in optimized_atoms to match the order in refined_atoms.
    Returns a list of tuples: (symbol, np.array([x, y, z]))
    """
    reordered_atoms = []
    refined_symbols = [atom[0] for atom in refined_atoms]
    optimized_symbols = [atom[0] for atom in optimized_atoms]

    if set(refined_symbols) != set(optimized_symbols):
        raise ValueError("The two structures have different sets of atomic symbols.")

    # Create a list to track which atoms have been matched
    matched_indices = []

    for symbol in refined_symbols:
        # Find all indices in optimized_symbols that match the current symbol
        matching_indices = [i for i, opt_symbol in enumerate(optimized_symbols) if opt_symbol == symbol and i not in matched_indices]

        if not matching_indices:
            raise ValueError(f"No matching atom found for symbol {symbol} in the optimized structure.")

        # Use the first matching index (you can add more logic here if needed)
        matched_index = matching_indices[0]
        matched_indices.append(matched_index)
        reordered_atoms.append(optimized_atoms[matched_index])

    return reordered_atoms

def main():
    Refined_stru = '3g2k_pdb.gjf'
    optimized_gjf = 'NH2_NH2_SH.gjf'
    output_gjf = 'aligned_QM-optimized.gjf'
    output_xyz = 'aligned_QM-optimized.xyz'
    PDB_Redo_XYZ = 'PDB_Redo_XYZ.xyz'
    ...
    # Reorder the optimized atoms to match the refined atoms
    try:
        optimized_atoms = reorder_atoms(refined_atoms, optimized_atoms)
    except ValueError as ve:
        print(f"Error during reordering: {ve}")
        return
    ...
    write_xyz(PDB_Redo_XYZ, refined_atoms, comment='Refined Structure')
    print(f"Refined structure has been written to {PDB_Redo_XYZ}")
